...
\tadOperacion{hippieNatural}{as/a,pos/p}{nat}{posValida(campus(as),p)}
\tadOperacion{hippiesCercanos}{as/a,pos/p}{nat}{posValida(campus(as),p)}
...
\tadAxioma{maxCapturas(a,segs)}{\IF $\emptyset?(segs)$ THEN 
								...
								ELSE
									...
									FI}
								FI}
\tadAxioma{hippieNatural($a,p$)}{\IF hayHippie?(a,p) THEN 1 ELSE 0 FI}
\tadAxioma{hippiesCercanos($a,p$)}{{\IF posValida(a,<$\pi_1(p)+1,\pi_2(p)$>) THEN hippieNatural((a,<$\pi_1(p)+1,\pi_2(p)$>) ELSE 0 FI}+{\IF posValida(a,<$\pi_1(p)-1,\pi_2(p)$>) THEN hippieNatural((a,<$\pi_1(p)-1,\pi_2(p)$>) ELSE 0 FI}+{\IF posValida(a,<$\pi_1(p),\pi_2(p)+1$>) THEN hippieNatural((a,<$\pi_1(p),\pi_2(p)+1$>) ELSE 0 FI}+{\IF posValida(a,<$\pi_1(p),\pi_2(p)-1$>) THEN hippieNatural((a,<$\pi_1(p),\pi_2(p)-1$>) ELSE 0 FI}}
...
\tadIgualdadObservacional{c}{c'}{campus}{alto(c)=alto(c') $/land$ ancho(c)=ancho(c') $/land$ obstaculos(c)=obstaculos(c')}
...
\tadOperacion{posValida}{campus/c,pos/p}{bool}{posValida(c,p)}
