...
\tadAxioma{hipEncerradoSeg?(a,p,adys)}{\IF $\emptyset?(adys)$ THEN $True$ ELSE 
											{\IF $posValida?(campus(a),dameUno(adys))$ THEN 
												$haySeg?(a,p)$ $\land$ $hipEncerradoSeg?(a,p,sinUno(adys))$
											ELSE 
												$False$
											FI}
									  FI}
...
\tadAxioma{proxPossHippies(a,possHippies)}{\IF $\emptyset?(possHippies)$ THEN $\emptyset$
										   ELSE 
										    	$proxPoss(a,estsCerca(dameUno(possHippies),dameUno(possHippies))) \cup $
										    	\\$proxPossHippies(a,sinUno(possHippies))$
										   FI}
...
\tadAxioma{conjPos(c,x,y)}{\IF $x \ge ancho(c)$ THEN $\emptyset$ ELSE
								...
								FI}
							FI}
\tadAxioma{adyacentes(campus,p)}{$\{<\pi_1(p)+1,\pi_2(p)+1><\pi_1(p)-1,\pi_2(p)-1><\pi_1(p)+1,\pi_2(p)><\pi_1(p),\pi_2(p)+1> \}$}
